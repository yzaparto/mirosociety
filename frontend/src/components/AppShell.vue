<template>
  <div class="min-h-screen flex flex-col bg-[#fafafa]">
    <nav
      v-if="!hideShell"
      class="h-12 border-b border-gray-200 flex items-center px-5 gap-4 shrink-0 bg-white/80 backdrop-blur-sm sticky top-0 z-40"
    >
      <router-link to="/" class="flex items-center gap-2 group">
        ...
        <span class="font-semibold text-sm text-slate-800 group-hover:text-slate-900 transition-colors">MiroSociety</span>
      </router-link>
      ...
      <router-link
        ...
        class="flex items-center gap-2 text-[11px] text-emerald-600 hover:text-emerald-500 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-1.5 transition-colors"
      >
      ...
      <router-link
        ...
        :class="[
          ...
          isGallery ? 'text-slate-900 bg-gray-100' : 'text-slate-500 hover:text-slate-800 hover:bg-gray-50'
        ]"
      >
        Gallery
      </router-link>
